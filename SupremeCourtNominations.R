...
promotions = subset(succession, Vacancy.Reason == 'Promoted')
...
promotions$YearNominee = paste(format(promotions$Vacancy.Date, '%Y'), promotions$Vacancy.Name)
nominations$YearNominee = paste(format(nominations$Result.Date, '%Y'), nominations$Nominee)
joined = merge(nominations, promotions, all.x=T)
joined = subset(joined, is.na(Vacancy.Reason))
...
p = ggplot(joined) +
    geom_rect(ymin=95,ymax=nrow(joined),xmin=maketerm(as.Date("2015-11-01")),xmax=maketerm(as.Date("2016-11-01")),color="transparent",fill="gray",alpha=0.5) +
    ...
    scale_y_continuous(breaks=c(),limits=c(96,NA)) +
    labs(title="Historical Timing of US Supreme Court Nominations Relative to Presidential Term\n", x="Years into term", y="")
